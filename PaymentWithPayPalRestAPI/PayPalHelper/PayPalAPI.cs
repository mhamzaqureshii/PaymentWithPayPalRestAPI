...
namespace PaymentWithPayPalRestAPI.PayPalHelper
{
        ...
        public async Task<string> getRedirectURLToPayPal(double total, string currency)
        {
            try
            {
                return Task.Run(async () =>
                {
                    ...
                    PayPalPaymentCreatedResponse createdResponse = await CreatePaypalPaymentAsync(http, accessToken, total, currency);
